...
export async function GET() {
  ...
  try {
    ...
    const actualMatch = pageText.match(/held rates steady at\s+[\d.]+?%–([\d.]+?)%/i);
    ...
    if (actualValue !== null && pageText.includes('as expected')) {
      ...
      const forecastExplicitMatch = pageText.match(/(?:market expectations of|forecasts of)\s+([\d.]+?)%/i);
    ...
    if (actualValue === null || forecastValue === null) {
      console.warn('No se pudieron encontrar ambos valores (actual y previsión) para la Tasa de Interés.');
