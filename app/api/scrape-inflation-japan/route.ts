// app/api/scrape-inflation-japan/route.ts (Para Next.js App Router)
...
export async function GET() {
  const url = "https://tradingeconomics.com/japan/inflation-cpi"; // URL de Inflación de Japón
  ...
  try {
    ...
    // Buscar patrones como "inflation rate eased to X%" o "at X%"
    // El texto de ejemplo dice "inflation rate eased to 3.3% in June 2025"
    const actualMatch = pageText.match(
      /(?:inflation rate eased to|at)\s+([\d.]+?)%/i
    );
    if (actualMatch && actualMatch[1]) {
      actualValue = parseFloat(actualMatch[1]);
      // Para depuración: console.log("Actual Match (JP Inflation):", actualMatch[1], "Parsed:", actualValue);
    }

    // --- Extracción de forecastValue ---
    // El texto de ejemplo dice "from 3.5% in the previous month" (esto sería el valor anterior, no una previsión)
    // También menciona "matching the headline rate at 3.3%, pointing to a three-month low following May's 3.7%."
    // Para la previsión, nos basaremos más en la tabla si no hay un "forecast" explícito en el texto.
    ...
    if (actualValue === null) {
      ...
      console.warn(
        "No se pudo encontrar el valor actual para Inflación de Japón. Actual:",
        actualValue,
        "Forecast:",
        forecastValue
      );
      return NextResponse.json<ScrapedData>(
        {
          error: "No se pudieron extraer los datos de Inflación de Japón.",
          variable: "Inflación",
          actualValue: null,
          forecastValue: null,
        },
    ...
    console.error(
      "Error al hacer scraping de Inflación de Japón:",
      errorMessage
    );
    return NextResponse.json<ScrapedData>(
      {
        error: `Fallo al obtener datos de Inflación de Japón: ${errorMessage}`,
        variable: "Inflación",
        actualValue: null,
        forecastValue: null,
      },
