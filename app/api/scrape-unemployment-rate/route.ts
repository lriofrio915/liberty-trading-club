...
export async function GET() {
  ...
  try {
    ...
    const actualMatch = pageText.match(/unemployment rate (?:rose slightly to|was)\s+([\d.]+?)%/i);
    ...
    if (actualValue !== null && pageText.includes('aligning with market expectations')) {
      ...
      const forecastExplicitMatch = pageText.match(/(?:market expectations of|forecasts of)\s+a\s+([\d.]+?)%/i);
    ...
    if (actualValue === null || forecastValue === null) {
      console.warn('No se pudieron encontrar ambos valores (actual y previsión) para la Tasa de Desempleo.');
    ...
    console.error('Error al hacer scraping de Tasa de Desempleo:', errorMessage);
    return NextResponse.json<ScrapedData>( // Tipado explícito de la respuesta
      { 
