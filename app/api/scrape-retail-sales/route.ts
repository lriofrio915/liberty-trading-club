...
export async function GET() {
  ...
  try {
    ...
    // Busca "increased X%" o "rose X%"
    const actualMatch = pageText.match(/(?:increased|rose)\s+([\d.]+?)%/i);
    if (actualMatch && actualMatch[1]) {
      actualValue = parseFloat(actualMatch[1]);
      // Para depuración: console.log("Actual Match (Retail Sales):", actualMatch[1], "Parsed:", actualValue);
    }

    // Expresión regular para encontrar la previsión:
    // Busca "expectations of a Y% gain"
    const forecastMatch = pageText.match(/expectations of a\s+([\d.]+?)%\s+gain/i);
    if (forecastMatch && forecastMatch[1]) {
      forecastValue = parseFloat(forecastMatch[1]);
      // Para depuración: console.log("Forecast Match (Retail Sales):", forecastMatch[1], "Parsed:", forecastValue);
    }
    ...
    if (actualValue === null || forecastValue === null) {
      console.warn('No se pudieron encontrar ambos valores (actual y previsión) para las Ventas Minoristas.');
    ...
    console.error('Error al hacer scraping de Ventas Minoristas:', errorMessage);
    return NextResponse.json<ScrapedData>( // Tipado explícito de la respuesta
      { 
